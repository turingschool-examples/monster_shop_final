...
describe Discount do
  ...
  describe 'class methods' do
    it '.item_discount' do
      ...
      order_quantity = 4
      expect(Discount.item_discount(ogre.id, ogre.price, order_quantity)).to eq(0)

      order_quantity = 5
      expect(Discount.item_discount(ogre.id, ogre.price, order_quantity).round(2)).to eq(5)

      order_quantity = 12
      expect(Discount.item_discount(ogre.id, ogre.price, order_quantity).round(2)).to eq(30)
      
      expect(Discount.item_discount(giant.id, giant.price, order_quantity)).to eq(0)
    end
  end
end
