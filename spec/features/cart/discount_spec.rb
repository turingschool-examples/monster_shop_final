...
RSpec.describe 'Cart Show Page' do
  describe 'As a regular user' do
    before :each do
      ...
      @bird = @brian.items.create!(name: 'Bird', description: "I'm a Bird!", price: 10, image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTaLM_vbg2Rh-mZ-B4t-RSU9AmSfEEq_SN9xPP_qrA2I6Ftq_D9Qw', active: true, inventory: 50 )
    ...
    describe 'I can have discounts applied' do
      ...
      it 'discounts default to the largest applicable discount' do
      ...
      it 'only for items from merchants offering the discount' do
        discount_1 = Discount.create!(quantity: 5, percentage: 10, merchant_id: @brian.id)
      ...
      it 'only for items that reach the quantity threshold' do
        discount_1 = Discount.create!(quantity: 5, percentage: 10, merchant_id: @brian.id)

        visit item_path(@bird)
        click_button 'Add to Cart'
        visit item_path(@hippo)
        click_button 'Add to Cart'
